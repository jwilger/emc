...
- 2026-06-03: `emc review record --workflow <slug> --reviewer <reviewer> --reviewed-at <timestamp>` and MCP `record_clean_review` now create deterministic clean review records for the current workflow digest. Reviewer and timestamp values are parsed at the boundary so the core remains clock-free.
