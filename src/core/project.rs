...
use crate::core::formal_model::{
    FormalModelSlice, FormalModelSliceModule, FormalModelWorkflow,
    lean_automation_record_structures, lean_board_record_structures,
    lean_command_error_record_structure, lean_command_input_record_structure,
    lean_command_record_structure, lean_data_flow_record_structure,
    lean_event_inventory_record_structures, lean_external_payload_record_structures,
    lean_model_slice_list, lean_model_slice_module_list, lean_model_workflow_list,
    lean_outcome_record_structure, lean_read_model_record_structures,
    lean_scenario_record_structures, lean_slice_record_structures,
    lean_translation_record_structures, lean_view_record_structures,
    lean_workflow_record_structure, quint_model_slice_list, quint_model_slice_module_list,
};
...
fn emit_lean_project_root(
    ...
    let module_name = module_name(project_name);
    let model_workflows = formal_model_workflows(workflow_slugs);
    let workflow_list = lean_model_workflow_list(&model_workflows);
    ...
    let model_workflow_structure = lean_workflow_record_structure();
    ...
    let model_board_structures = lean_board_record_structures();
    let model_automation_structures = lean_automation_record_structures();
    let model_translation_structures = lean_translation_record_structures();
    let model_external_payload_structures = lean_external_payload_record_structures();
...
fn emit_quint_project_root(
    ...
    .replace("dataFlow.sourceKind != \"\" and ", "")
    .replace(
        "dataFlow.sourceKind == \"original\"",
        "dataFlow.sourceKind == Original",
    )
    .replace(
        "dataFlow.sourceKind == \"modeled_target\"",
        "dataFlow.sourceKind == ModeledTarget",
    )
    .replace(
        "dataFlow.sourceKind != \"modeled_target\"",
        "dataFlow.sourceKind != ModeledTarget",
...
fn formal_model_workflows(workflow_slugs: &[WorkflowSlug]) -> Vec<FormalModelWorkflow> {
    workflow_slugs
        .iter()
        .cloned()
        .map(FormalModelWorkflow::new)
        .collect()
}
