...
impl ExportedEventBody {
    ...
    fn slice_payload_json(&self) -> Value {
        match self {
            ...
            Self::SliceCommandDefinitionRemoved { slice, name } => {
                json!({ "slice": slice.as_ref(), "name": name.as_ref() })
            }
            Self::SliceReadModelAdded { read_model }
            | Self::SliceReadModelDefinitionUpdated { read_model } => {
            ...
            Self::SliceViewControlUpdated {
                ...
                "slice": slice.as_ref(),
                "view": view.as_ref(),
                "control": SliceViewAddedEventPayload::control_to_json_value(control),
            }),
            Self::SliceViewControlRemoved { slice, view, name } => {
                json!({
                    "slice": slice.as_ref(),
                    "view": view.as_ref(),
                    "name": name.as_ref(),
    ...
    fn board_element_payload_json(&self) -> Value {
        match self {
            Self::SliceBoardElementAdded { element }
            | Self::SliceBoardElementUpdated { element } => {
    ...
    fn slice_from_event_type_and_payload(
        ...
        match event_type {
            ...
            ExportedEventType::SliceCommandDefinitionUpdated => {
                Ok(Self::SliceCommandDefinitionUpdated {
                    command: SliceCommandDefinitionAddedEventPayload::from_json_value(payload)?
                        .into_command(),
                })
            }
            ExportedEventType::SliceCommandDefinitionRemoved => {
                Ok(Self::SliceCommandDefinitionRemoved {
                    slice: slice_slug(required_str(payload, "slice")?)?,
                    name: command_name(required_str(payload, "name")?)?,
                })
            }
            ExportedEventType::SliceReadModelAdded
            | ExportedEventType::SliceReadModelDefinitionUpdated
            | ExportedEventType::SliceReadModelDefinitionRemoved => {
                Self::read_model_from_event_type_and_payload(event_type, payload)
            }
            ExportedEventType::SliceViewAdded
            | ExportedEventType::SliceViewDefinitionUpdated
            | ExportedEventType::SliceViewDefinitionRemoved
            | ExportedEventType::SliceViewControlUpdated
            ...
            | ExportedEventType::SliceBoardElementUpdated
    ...
    fn board_element_from_event_type_and_payload(
        ...
        match event_type {
            ...
            ExportedEventType::SliceBoardElementUpdated => Ok(Self::SliceBoardElementUpdated {
                element: SliceBoardElementAddedEventPayload::from_json_value(payload)?
                    .into_element(),
            }),
            ExportedEventType::SliceBoardElementRemoved => Ok(Self::SliceBoardElementRemoved {
                slice: slice_slug(required_str(payload, "slice")?)?,
                name: board_element_name(required_str(payload, "name")?)?,
            }),
            other => Err(format!(
    ...
    fn view_from_event_type_and_payload(
        event_type: ExportedEventType,
        payload: &Value,
    ) -> Result<Self, String> {
        match event_type {
            ...
            ExportedEventType::SliceViewControlUpdated => Ok(Self::SliceViewControlUpdated {
                slice: slice_slug(required_str(payload, "slice")?)?,
                view: view_name(required_str(payload, "view")?)?,
                control: SliceViewAddedEventPayload::control_definition_from_json_value(
                    required_object(payload, "control")?,
                )?,
            }),
            ExportedEventType::SliceViewControlRemoved => Ok(Self::SliceViewControlRemoved {
                slice: slice_slug(required_str(payload, "slice")?)?,
                view: view_name(required_str(payload, "view")?)?,
                name: control_name(required_str(payload, "name")?)?,
            }),
            _ => Err(format!("unexpected view event type {event_type}")),
        }
    }

    fn read_model_from_event_type_and_payload(
        event_type: ExportedEventType,
        payload: &Value,
    ) -> Result<Self, String> {
        match event_type {
            ...
            ExportedEventType::SliceReadModelDefinitionUpdated => {
                Ok(Self::SliceReadModelDefinitionUpdated {
                    read_model: SliceReadModelAddedEventPayload::from_json_value(payload)?
                        .into_read_model(),
                })
            }
            ExportedEventType::SliceReadModelDefinitionRemoved => {
                Ok(Self::SliceReadModelDefinitionRemoved {
                    slice: slice_slug(required_str(payload, "slice")?)?,
                    name: read_model_name(required_str(payload, "name")?)?,
                })
            }
            _ => Err(format!("unexpected read model event type {event_type}")),
        }
    }
...
impl EventDraft {
    ...
    pub(crate) fn slice_command_definition_updated(command: &NewCommandDefinition) -> Self {
        Self {
            stream_id: EventStreamId::slice(command.slice_slug()),
            body: ExportedEventBody::SliceCommandDefinitionUpdated {
                command: command.clone(),
            },
        }
    }

    pub(crate) fn slice_command_definition_removed(slice: &SliceSlug, name: &CommandName) -> Self {
        Self {
            stream_id: EventStreamId::slice(slice),
            body: ExportedEventBody::SliceCommandDefinitionRemoved {
                slice: slice.clone(),
                name: name.clone(),
            },
        }
    ...
    pub(crate) fn slice_read_model_definition_updated(read_model: &NewReadModelDefinition) -> Self {
        Self {
            stream_id: EventStreamId::slice(read_model.slice_slug()),
            body: ExportedEventBody::SliceReadModelDefinitionUpdated {
                read_model: read_model.clone(),
            },
        }
    }

    pub(crate) fn slice_read_model_definition_removed(
        slice: &SliceSlug,
        name: &ReadModelName,
    ) -> Self {
        Self {
            stream_id: EventStreamId::slice(slice),
            body: ExportedEventBody::SliceReadModelDefinitionRemoved {
                slice: slice.clone(),
                name: name.clone(),
            },
        }
    ...
    pub(crate) fn slice_view_control_updated(
        slice: &SliceSlug,
        view: &ViewName,
        control: &NewControlDefinition,
    ) -> Self {
        Self {
            stream_id: EventStreamId::slice(slice),
            body: ExportedEventBody::SliceViewControlUpdated {
                slice: slice.clone(),
                view: view.clone(),
    ...
    pub(crate) fn slice_view_control_removed(
        slice: &SliceSlug,
        view: &ViewName,
        name: &ControlName,
    ) -> Self {
        Self {
            stream_id: EventStreamId::slice(slice),
            body: ExportedEventBody::SliceViewControlRemoved {
                slice: slice.clone(),
                view: view.clone(),
                name: name.clone(),
            },
    ...
    pub(crate) fn slice_board_element_updated(element: &NewBoardElement) -> Self {
        Self {
            stream_id: EventStreamId::slice(element.slice_slug()),
            body: ExportedEventBody::SliceBoardElementUpdated {
                element: element.clone(),
            },
        }
    }

    pub(crate) fn slice_board_element_removed(slice: &SliceSlug, name: &BoardElementName) -> Self {
        Self {
            stream_id: EventStreamId::slice(slice),
            body: ExportedEventBody::SliceBoardElementRemoved {
                slice: slice.clone(),
                name: name.clone(),
            },
        }
...
impl ProjectedModel {
    ...
    fn apply_event(model: Option<Self>, event: EmcEvent) -> Result<Option<Self>, String> {
        match event {
            ...
            EmcEvent::SliceCommandDefinitionUpdated { .. }
            ...
            EmcEvent::SliceViewControlUpdated { control, .. } => Self::apply_view_control_updated(
                model,
                control.slice(),
                control.view(),
                control.control(),
            ),
            EmcEvent::SliceViewControlRemoved { control, .. } => Self::apply_view_control_removed(
                model,
                control.slice(),
                control.view(),
                control.name(),
    ...
    fn apply_board_element_event(
        model: Option<Self>,
        event: EmcEvent,
    ) -> Result<Option<Self>, String> {
        match event {
            EmcEvent::SliceBoardElementUpdated { element, .. } => {
                Self::apply_board_element_updated(model, element.element())
            }
            EmcEvent::SliceBoardElementRemoved { element, .. } => {
    ...
    fn apply_command_definition_event(
        model: Option<Self>,
        event: EmcEvent,
    ) -> Result<Option<Self>, String> {
        match event {
            ...
            EmcEvent::SliceCommandDefinitionRemoved { command, .. } => {
                Self::apply_command_definition_removed(model, command.slice(), command.name())
            }
            _ => Err("apply_command_definition_event received a non-command event".to_owned()),
        }
    }
    ...
    fn apply_slice_scenario_event(
        ...
        match event {
            ...
            _ => Err("apply_slice_scenario_event received a non-scenario event".to_owned()),
        }
    }
    ...
    fn apply_board_element_updated(
        model: Option<Self>,
        element: NewBoardElement,
    ) -> Result<Option<Self>, String> {
        let mut model = Self::require(model, "SliceBoardElementUpdated")?;
        model.apply_slice_fact_body(ExportedEventBody::SliceBoardElementUpdated { element })?;
        Ok(Some(model))
    }

    fn apply_board_element_removed(
        model: Option<Self>,
        slice: &SliceSlug,
        name: &BoardElementName,
    ) -> Result<Option<Self>, String> {
        let mut model = Self::require(model, "SliceBoardElementRemoved")?;
        model.apply_slice_fact_body(ExportedEventBody::SliceBoardElementRemoved {
            slice: slice.clone(),
            name: name.clone(),
        })?;
        Ok(Some(model))
    ...
    fn apply_command_definition_removed(
        model: Option<Self>,
        slice: &SliceSlug,
        name: &CommandName,
    ) -> Result<Option<Self>, String> {
        let mut model = Self::require(model, "SliceCommandDefinitionRemoved")?;
        model.apply_slice_fact_body(ExportedEventBody::SliceCommandDefinitionRemoved {
            slice: slice.clone(),
            name: name.clone(),
        })?;
        Ok(Some(model))
    ...
    fn apply_read_model_definition_updated(
        model: Option<Self>,
        read_model: NewReadModelDefinition,
    ) -> Result<Option<Self>, String> {
        let mut model = Self::require(model, "SliceReadModelDefinitionUpdated")?;
        model.apply_slice_fact_body(ExportedEventBody::SliceReadModelDefinitionUpdated {
            read_model,
        })?;
        Ok(Some(model))
    }

    fn apply_read_model_definition_removed(
        model: Option<Self>,
        slice: &SliceSlug,
        name: &ReadModelName,
    ) -> Result<Option<Self>, String> {
    ...
    fn apply_view_control_updated(
        model: Option<Self>,
        slice: &SliceSlug,
        view: &ViewName,
        control: NewControlDefinition,
    ) -> Result<Option<Self>, String> {
        let mut model = Self::require(model, "SliceViewControlUpdated")?;
        model.apply_slice_fact_body(ExportedEventBody::SliceViewControlUpdated {
            slice: slice.clone(),
            view: view.clone(),
            control,
        })?;
    ...
    fn apply_view_control_removed(
        model: Option<Self>,
        slice: &SliceSlug,
        view: &ViewName,
        name: &ControlName,
    ) -> Result<Option<Self>, String> {
        let mut model = Self::require(model, "SliceViewControlRemoved")?;
        model.apply_slice_fact_body(ExportedEventBody::SliceViewControlRemoved {
            slice: slice.clone(),
            view: view.clone(),
            name: name.clone(),
        })?;
    ...
    fn apply_slice_fact_body(&mut self, body: ExportedEventBody) -> Result<(), String> {
        match body {
            ...
            ExportedEventBody::SliceCommandDefinitionUpdated { command } => {
                self.apply_slice_command_definition_updated(command)?;
            }
            ExportedEventBody::SliceCommandDefinitionRemoved { slice, name } => {
                self.apply_slice_command_definition_removed(&slice, &name)?;
            }
            ...
            ExportedEventBody::SliceReadModelDefinitionUpdated { read_model } => {
                self.apply_slice_read_model_definition_updated(read_model)?;
            }
    ...
    fn apply_slice_board_element_body(&mut self, body: ExportedEventBody) -> Result<(), String> {
        match body {
            ...
            ExportedEventBody::SliceBoardElementUpdated { element } => {
                let slice = self.slice_mut(element.slice_slug(), "SliceBoardElementUpdated")?;
                slice
                    .board_elements
            ...
            ExportedEventBody::SliceBoardElementRemoved { slice, name } => {
                self.slice_mut(&slice, "SliceBoardElementRemoved")?
                    .board_elements
                    .retain(|element| element.name() != &name);
            }
            other => {
                return Err(format!(
                    "apply_slice_board_element_body received a non-board-element body {}",
                    other.event_type()
                ));
            }
        }
    ...
    fn apply_slice_view_body(&mut self, body: ExportedEventBody) -> Result<(), String> {
        match body {
            ...
            ExportedEventBody::SliceViewControlUpdated {
                ...
                self.apply_slice_view_control_updated(&slice, &view, control)?;
            }
            ExportedEventBody::SliceViewControlRemoved { slice, view, name } => {
                self.apply_slice_view_control_removed(&slice, &view, &name)?;
            }
            _ => return Err("apply_slice_view_body received a non-view body".to_owned()),
        }
        Ok(())
    }
    ...
    fn apply_slice_command_definition_updated(
        ...
        let slice = self.slice_mut(command.slice_slug(), "SliceCommandDefinitionUpdated")?;
        slice
            .command_definitions
    ...
    fn apply_slice_command_definition_removed(
        &mut self,
        slice: &SliceSlug,
        name: &CommandName,
    ) -> Result<(), String> {
        self.slice_mut(slice, "SliceCommandDefinitionRemoved")?
            .command_definitions
            .retain(|command| command.name() != name);
        Ok(())
    }

    fn apply_slice_read_model_definition_updated(
        &mut self,
        read_model: NewReadModelDefinition,
    ) -> Result<(), String> {
        let slice = self.slice_mut(read_model.slice_slug(), "SliceReadModelDefinitionUpdated")?;
        slice
            .read_models
            .retain(|existing| existing.name() != read_model.name());
        slice.read_models.push(read_model);
        Ok(())
    }

    fn apply_slice_read_model_definition_removed(
        &mut self,
        slice: &SliceSlug,
        name: &ReadModelName,
    ) -> Result<(), String> {
    ...
    fn apply_slice_view_control_updated(
        &mut self,
        slice: &SliceSlug,
        view: &ViewName,
        control: NewControlDefinition,
    ) -> Result<(), String> {
        let slice = self.slice_mut(slice, "SliceViewControlUpdated")?;
        let view_definition = slice
            .views
            .iter_mut()
            .find(|existing| existing.name() == view)
            .ok_or_else(|| {
                format!(
                    "SliceViewControlUpdated requires existing view '{}' in slice '{}'",
                    view.as_ref(),
                    slice.slug.as_ref()
                )
            })?;
        *view_definition = view_definition.clone().with_updated_control(control);
        Ok(())
    }

    fn apply_slice_view_control_removed(
        &mut self,
        slice: &SliceSlug,
        view: &ViewName,
        name: &ControlName,
    ) -> Result<(), String> {
        let slice = self.slice_mut(slice, "SliceViewControlRemoved")?;
        let view_definition = slice
            .views
            .iter_mut()
            .find(|existing| existing.name() == view)
            .ok_or_else(|| {
                format!(
                    "SliceViewControlRemoved requires existing view '{}' in slice '{}'",
                    view.as_ref(),
                    slice.slug.as_ref()
                )
            })?;
        *view_definition = view_definition.clone().with_removed_control(name);
        Ok(())
    }
