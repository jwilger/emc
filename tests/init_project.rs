    ...
    fn init_creates_deterministic_project_layout() -> Result<(), Box<dyn Error>> {
        ...
        assert!(
            fs::read_to_string(temp_dir.path().join("model/lean/RepairDesk.lean"))?.contains(
                "def modelDigest := \"project:name=Repair Desk;version=0.1.0;workflows=;slices=;commands=;read-models=;views=;automations=;streams=;events=\""
            ),
        ...
        assert!(
            fs::read_to_string(temp_dir.path().join("model/lean/RepairDesk.lean"))?.contains(
                "theorem modelDigestIsStable : modelDigest = \"project:name=Repair Desk;version=0.1.0;workflows=;slices=;commands=;read-models=;views=;automations=;streams=;events=\" := rfl"
            ),
        ...
        assert!(
            fs::read_to_string(temp_dir.path().join("model/quint/RepairDesk.qnt"))?.contains(
                "val modelDigest = \"project:name=Repair Desk;version=0.1.0;workflows=;slices=;commands=;read-models=;views=;automations=;streams=;events=\""
            ),
        ...
        assert!(
            fs::read_to_string(temp_dir.path().join("model/quint/RepairDesk.qnt"))?.contains(
                "val modelDigestStable = modelDigest == \"project:name=Repair Desk;version=0.1.0;workflows=;slices=;commands=;read-models=;views=;automations=;streams=;events=\""
            ),
