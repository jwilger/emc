...
mod tests {
    ...
    fn add_automation_definition_updates_formal_slice_artifacts() -> Result<(), Box<dyn Error>> {
        ...
        assert!(
            lean_root.contains(
                "def modelDigest := \"project:name=Repair Desk;version=0.1.0;workflows=open-ticket;slices=open-ticket/capture-ticket@CaptureTicket;scenarios=;data-flows=;outcomes=;command-errors=;commands=;command-inputs=;read-models=;read-model-definitions=;read-model-fields=;views=;view-fields=;automations=open-ticket/capture-ticket/title-deduplicator;translations=;external-payloads=;streams=;events=;event-attributes=\""
            ),
            "Lean project root digest must include authored automation inventory"
        );
        assert!(
            quint_root.contains(
                "val modelDigest = \"project:name=Repair Desk;version=0.1.0;workflows=open-ticket;slices=open-ticket/capture-ticket@CaptureTicket;scenarios=;data-flows=;outcomes=;command-errors=;commands=;command-inputs=;read-models=;read-model-definitions=;read-model-fields=;views=;view-fields=;automations=open-ticket/capture-ticket/title-deduplicator;translations=;external-payloads=;streams=;events=;event-attributes=\""
            ),
