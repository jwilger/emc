...
mod tests {
    ...
    fn check_reports_project_root_digest_drift() -> Result<(), Box<dyn Error>> {
        ...
        write(
            &lean_root_path,
            lean_root.replace(
                "def modelDigest := \"project:name=Repair Desk;version=0.1.0;workflows=open-ticket;slices=;commands=;streams=;events=\"",
                "def modelDigest := \"project:name=Repair Desk;version=0.1.0;workflows=stale;slices=;commands=;streams=;events=\"",
            ),
        )?;
