...
mod tests {
    ...
    fn add_slice_updates_workflow_composition_and_slice_data() -> Result<(), Box<dyn Error>> {
        ...
        assert!(
            lean_root.contains(
                "def modelDigest := \"project:name=Repair Desk;version=0.1.0;workflows=open-ticket;slices=open-ticket/capture-ticket@CaptureTicket;scenarios=;outcomes=;commands=;read-models=;views=;automations=;translations=;external-payloads=;streams=;events=\""
            ),
            "Lean project root digest must include composed workflow, slice, and module membership"
        );
        assert!(
            lean_root.contains(
                "theorem modelDigestIsStable : modelDigest = \"project:name=Repair Desk;version=0.1.0;workflows=open-ticket;slices=open-ticket/capture-ticket@CaptureTicket;scenarios=;outcomes=;commands=;read-models=;views=;automations=;translations=;external-payloads=;streams=;events=\" := rfl"
            ),
        ...
        assert!(
            quint_root.contains(
                "val modelDigest = \"project:name=Repair Desk;version=0.1.0;workflows=open-ticket;slices=open-ticket/capture-ticket@CaptureTicket;scenarios=;outcomes=;commands=;read-models=;views=;automations=;translations=;external-payloads=;streams=;events=\""
            ),
            "Quint project root digest must include composed workflow, slice, and module membership"
        );
        assert!(
            quint_root.contains(
                "val modelDigestStable = modelDigest == \"project:name=Repair Desk;version=0.1.0;workflows=open-ticket;slices=open-ticket/capture-ticket@CaptureTicket;scenarios=;outcomes=;commands=;read-models=;views=;automations=;translations=;external-payloads=;streams=;events=\""
            ),
